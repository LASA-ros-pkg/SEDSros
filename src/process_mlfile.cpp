...
bool process_dataset_manager_file(string filename, vector< vector<fvec> > &trajectories, ivec &labels, float &dT)
{
  ...
  int count = mlfile_options.count;
  int resampleType = mlfile_options.resampleType;
  int centerType = mlfile_options.centerType;
  bool zeroEnding = mlfile_options.zeroEnding;
  dT = mlfile_options.dT; // time span between each data frame
  float maxV = -FLT_MAX;
